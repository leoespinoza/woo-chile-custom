...
class WCSCL_Fields {
	...
	public function __construct() {
		$this->tabs = array( 'fields' => WOOCFCL_Utils::translate('Checkout Fields'));
		$this->sections = array(
	...
	public function set_after_setup_theme(){
		// Show in order details page
		add_action('after_setup_theme', array($this, 'define_admin_hooks'));
	...
	public function reset_to_default() {
		...
		echo '<div class="updated"><p>'. WOOCFCL_Utils::translate('SUCCESS: Checkout fields successfully reset') .'</p></div>';
	}
	...
	public function render_checkout_fields_heading_row(){
		?>
		<th class="sort"></th>
		<th class="check-column"><input type="checkbox" style="margin:0px 4px -1px -1px;" onclick="wcsclSelectAllCheckoutFields(this)"/></th>
		<th class="name"><?php WOOCFCL_Utils::etranslate('Name'); ?></th>
		<th class="name"><?php WOOCFCL_Utils::etranslate('Priority'); ?></th>
		<th class="id"><?php WOOCFCL_Utils::etranslate('Type'); ?></th>
		<th><?php WOOCFCL_Utils::etranslate('Label'); ?></th>
		<th><?php WOOCFCL_Utils::etranslate('Placeholder'); ?></th>
		<th><?php WOOCFCL_Utils::etranslate('Validations'); ?></th>
        <th class="status"><?php WOOCFCL_Utils::etranslate('Required'); ?></th>
		<th class="status"><?php WOOCFCL_Utils::etranslate('Enabled'); ?></th>	
        <th class="action"><?php WOOCFCL_Utils::etranslate('Edit'); ?></th>	
        <?php
	}
	...
	private function save_or_update_field($section, $action) {
		try {
			...
			if($name){
				if($action === 'new'){
					$priority = WCSCL_Utils::prepare_field_priority($fields, false, true);
					$field['custom'] = 1;
			...
			$result = WCSCL_Utils::update_fields($section, $fields);
			
			if($result == true) {
				echo '<div class="updated"><p>'. WOOCFCL_Utils::translate('Your changes were saved.') .'</p></div>';
			}else {
				echo '<div class="error"><p>'. WOOCFCL_Utils::translate('Your changes were not saved due to an error (or you made none!).') .'</p></div>';
			}
		} catch (Exception $e) {
			echo '<div class="error"><p>'. WOOCFCL_Utils::translate('Your changes were not saved due to an error.') .'</p></div>';
		}
	}
	...
	private function save_fields($section) {
		try {
			...
			if(empty($f_names)){
				echo '<div class="error"><p> '. WOOCFCL_Utils::translate('Your changes were not saved due to no fields found.') .'</p></div>';
				return;
			}
			...
			for($i = 0; $i <= $max; $i++) {
				...
				if(isset($fields[$name])){
					...
					$priority = WCSCL_Utils::prepare_field_priority($fields, $order, false);
			...
			$fields = WCSCL_Utils::sort_fields($fields);
			$result = WCSCL_Utils::update_fields($section, $fields);
	...
	private function prepare_field_from_posted_data($posted){
		...
		if($type === 'select' || $type === 'radio'){
			...
			$options_arr = WCSCL_Utils::prepare_options_array($options_json);
	...
	public function get_field_form_props(){
		...
		return array(
			'type' 		  => array('type'=>'select', 'name'=>'type', 'label'=>'Type', 'required'=>1, 'options'=>$field_types, 
								'onchange'=>'wcsclFieldTypeChangeListner(this)'),
	...
	private function render_form_field_element_select($props){
		...
		if(isset($props['placeholder']) && $props['placeholder']){
			$field_attr .= ' data-placeholder="'.WOOCFCL_Utils::translate($props['placeholder']).'"';
		}

		if(isset($props['multiple']) && $props['multiple']){
			$field_attr .= ' multiple="multiple"';
			$field_attr .= ' class="wcscl-enhanced-multi-select"';
	...
	private function render_field_form_fragment_options(){
		...
		<tr class="row-options">
			...
			<td>
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="wcscl-option-list thpladmin-dynamic-row-table"><tbody>
					<tr>
						...
						<td class="action-cell"><a href="javascript:void(0)" onclick="wcsclAddNewOptionRow(this)" class="btn btn-blue" title="Add new option">+</a></td>
						<td class="action-cell"><a href="javascript:void(0)" onclick="wcsclRemoveOptionRow(this)" class="btn btn-red" title="Remove option">x</a></td>
	...
	public function get_admin_url($tab = false, $section = false){
		$url = 'admin.php?page='.WOOCFCL_ACTION_LINK;
		if($tab && !empty($tab)){
			$url .= '&tab='. $tab;
		}
