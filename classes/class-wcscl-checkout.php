...
if(!class_exists('WCSCL_Checkout')) :

class WCSCL_Checkout {
	...
	public function enqueue_styles_and_scripts() {
		if(is_checkout()){
			$in_footer = apply_filters( 'wcscl_enqueue_script_in_footer', true );
			$deps = array('jquery', 'select2');

			wp_register_script('wcscl-checkout-script', WCSCL_ASSETS_URL.'js/wcscl-checkout.js', $deps, WOOCFCL_VERSION, $in_footer);
			wp_enqueue_script('wcscl-checkout-script');
			wp_enqueue_style('wcscl-checkout-style', WCSCL_ASSETS_URL . 'css/wcscl-public.css', WOOCFCL_VERSION);
		}
	}

	public function define_public_hooks(){
		$hp_default_address_fields = apply_filters('wcscl_default_address_fields_priority', 1000);
		$hp_billing_fields  = apply_filters('wcscl_billing_fields_priority', 1000);
		$hp_shipping_fields = apply_filters('wcscl_shipping_fields_priority', 1000);
		$hp_checkout_fields = apply_filters('wcscl_checkout_fields_priority', 1000);
	...
	public function prepare_country_locale($fields) {
		if(is_array($fields)){
			$sname = apply_filters('wcscl_address_field_override_with', 'billing');
			$address_fields = get_option('wc_fields_'.$sname);

			foreach($fields as $key => $props){
				$override_ph = apply_filters('wcscl_address_field_override_placeholder', true);
				$override_label = apply_filters('wcscl_address_field_override_label', true);
				$override_required = apply_filters('wcscl_address_field_override_required', false);
				$override_priority = apply_filters('wcscl_address_field_override_priority', true);
	...
	public function default_address_fields($fields) {
		...
		if($sname === 'billing' || $sname === 'shipping'){
			...
			if(is_array($address_fields) && !empty($address_fields) && !empty($fields)){
				$override_required = apply_filters( 'wcscl_address_field_override_required', true );
				
				foreach($fields as $name => $field) {
					$fname = $sname.'_'.$name;
					
					if(WCSCL_Utils::is_address_field($fname) && $override_required){
						$custom_field = isset($address_fields[$fname]) ? $address_fields[$fname] : false;
						
						if(WCSCL_Utils::is_enabled($custom_field)){
	...
	public function prepare_checkout_fields($fields, $original_fields) {
		if(is_array($fields) && !empty($fields)) {
			foreach($fields as $name => $field) {
				if(WCSCL_Utils::is_enabled($field)) {
					$new_field = false;
					$allow_override = apply_filters('wcscl_allow_default_field_override_'.$name, false);
					...
					$new_field['class'][] = 'wcscl-field-wrapper';
					$new_field['class'][] = 'wcscl-field-'.$type;
					
					if($type === 'select' || $type === 'radio'){
						if(isset($new_field['options'])){
							$options_arr = WCSCL_Utils::prepare_field_options($new_field['options']);
							$options = array();
					...
					if($type === 'select' && apply_filters('wcscl_enable_select2_for_select_fields', true)){
						$new_field['input_class'][] = 'wcscl-enhanced-select';
					}
					
					if(isset($new_field['label'])){
						$new_field['label'] = WCSCL_Utils::t($new_field['label']);
					}

					if(isset($new_field['placeholder'])){
						$new_field['placeholder'] = WCSCL_Utils::t($new_field['placeholder']);
					}
	...
	*************************************/
		...
		if($key){
			...
			if($value && is_array($validators) && !empty($validators)){					
				foreach($validators as $vname){
					$err_msg = '';
					$flabel = isset($field['label']) ? WCSCL_Utils::t($field['label']) : $key;

					if($vname === 'number'){
						if(!is_numeric($value)){
							$err_msg = '<strong>'. $flabel .'</strong> '. WCSCL_Utils::t('is not a valid number.');	
						}
					}
	...
	public function checkout_update_order_meta($order_id, $posted){
		...
		foreach($types as $type){
			...
			$fields = WCSCL_Utils::get_fields($type);
			
			foreach($fields as $name => $field){
				if(WCSCL_Utils::is_active_custom_field($field) && isset($posted[$name])){
	...
	*****************************************/
		...
		foreach( $fields as $key => $field ) {
			if(isset($field['show_in_email']) && $field['show_in_email']){
				...
				if($value){
					...
					$custom_field = array();
					$custom_field['label'] = WCSCL_Utils::t($label);
	...
	public function order_details_after_customer_details($order){
		$order_id = WCSCL_Utils::get_order_id($order);
		$fields = WCSCL_Utils::get_checkout_fields($order);
		...
		if(is_array($fields) && !empty($fields)){
			...
			foreach($fields as $key => $field){			
				if(WCSCL_Utils::is_active_custom_field($field) && isset($field['show_in_order']) && $field['show_in_order']){
					...
					if($value){
						$label = isset($field['label']) && $field['label'] ? WCSCL_Utils::t($field['label']) : $key;
						...
						if(is_account_page()){
							if(apply_filters( 'wcscl_view_order_customer_details_table_view', true )){
								$fields_html .= '<tr><th>'. $label .':</th><td>'. $value .'</td></tr>';
							}else{
								$fields_html .= '<br/><dt>'. $label .':</dt><dd>'. $value .'</dd>';
							}
						}else{
							if(apply_filters( 'wcscl_thankyou_customer_details_table_view', true )){
								$fields_html .= '<tr><th>'. $label .':</th><td>'. $value .'</td></tr>';
							}else{
								$fields_html .= '<br/><dt>'. $label .':</dt><dd>'. $value .'</dd>';
							}
			...
			if($fields_html){
				do_action( 'wcscl_order_details_before_custom_fields_table', $order ); 
				?>
				...
				<?php
				do_action( 'wcscl_order_details_after_custom_fields_table', $order ); 
			}
		}
	}
	/*****************************************
